...
- (id)initWithSystemId:(uint8_t)systemId componentId:(uint8_t)componentId latitude:(int32_t)latitude longitude:(int32_t)longitude altitude:(int32_t)altitude x:(float)x y:(float)y z:(float)z q:(float *)q approachX:(float)approachX approachY:(float)approachY approachZ:(float)approachZ {
  if ((self = [super init])) {
    mavlink_msg_home_position_pack(systemId, componentId, &(self->_message), latitude, longitude, altitude, x, y, z, q, approachX, approachY, approachZ);
  }
  return self;
}
...
- (NSString *)description {
  return [NSString stringWithFormat:@"%@, latitude=%d, longitude=%d, altitude=%d, x=%f, y=%f, z=%f, q=%@, approachX=%f, approachY=%f, approachZ=%f", [super description], [self latitude], [self longitude], [self altitude], [self x], [self y], [self z], @"[array of float[4]]", [self approachX], [self approachY], [self approachZ]];
}
